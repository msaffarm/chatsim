from chatsim.utils import Annotation, Goal, get_random_number
...
class AgendaUser(object):
    ...
    def create_agenda(self, selected_goal_slots_with_intents, intent, domain):
        ...
        for intent, goal in selected_goal_slots_with_intents:
            ...
            if not goal.type == 'open':
                inform_slot_list.append(Annotation(diagact=Inform, goal_list=[copy.deepcopy(goal)],
            ...
            # only required slots with is_open flag
                ...
                if rand < self._profile["inform"]:
                    ...
                    request_slot_list.append(Annotation(diagact=Request,
                                                        goal_list=[Goal(slot=goal.slot, value=None, type=None)],
                                                        intent=intent, domain=domain))
    ...
    def next(self, sys_annotaions, turn_count):
        ...
        if self._max_turn > 0 and turn_count >= self._max_turn-1:
            ...
            if not episode_over:
                ...
                user_response += self._agenda.pop(number_of_items=self.number_of_items_to_pop_based_on_profile(self.current_agenda),
                                                  multi_diagact=False)
        ...
        for annot in annotation_list:
            if annot.goal_list and len(annot.goal_list) > 1:
                for gs in annot.goal_list:
                    single_slot_annotations.append(Annotation(diagact=annot.diagact, goal_list=[gs],
                                                              intent=annot.intent, domain='movie'))
            else:
    ...
    def response_to_request(self, sys_annotaion):
        ...
        if goal_slots_to_inform:
            response_list.append(Annotation(diagact=Inform, goal_list=goal_slots_to_inform,
                                            intent=intent, domain='movie'))

        if goal_slots_to_request_option:
            response_list.append(Annotation(diagact=RequestOptions(), goal_slot_list=goal_slots_to_request_option,
                                            intent=intent))
            ...
            if user_goal_slot_flag in [GOAL_SLOT_FLAGS.is_fixed, GOAL_SLOT_FLAGS.is_flexible]:
                ...
                if not user_goal_slot_values_set.issubset(slot_selected_values_set):
                    ...
                    response_list.append(Annotation(diagact=Inform(),
                                                    goal_slot_list=[GoalSlot(slot_name,
                                                                             list(user_goal_slot_values_set),
                                                                             user_goal_slot_flag)],
                                                    intent=intent))
    ...
    def response_to_notifysuccess(self, sys_annotaion):
        ...
        response_list = []
        intent = sys_annotaion.intent
        response_list.append(Annotation(diagact=ThankYou, intent=intent,
                                        domain='movie', goal_list=None))
